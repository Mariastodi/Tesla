...
const adminLimiter = rateLimit({
  ...
  message: {
    error: "Muitas tentativas de acesso. Aguarde 15 minutos.",
  },
  standardHeaders: "draft-7",
  legacyHeaders: false,
});
...
const requireAdmin = async (req, res, next) => {
  ...
  return res.status(401).json({
    error: "Acesso da coordenação não autorizado.",
  });
};
...
const deviceEnabled = (req) => {
  ...
  return (
    ...
    sameSecret(signature, signDevice(expires))
  );
};
...
app.use("/api/kiosk", (req, res, next) =>
  deviceEnabled(req)
    ...
    : res.status(401).json({
        error:
          "Este tablet precisa ser ativado pela coordenação.",
      }),
);
  ...
  try {
    ...
    res.json({
      ...
      config: {
        ...
        successScreenMs:
          store.config.successScreenMs || 5500,
      },
    ...
    res.status(500).json({
      error: "Não foi possível carregar o sistema.",
    });
  }
});
...
routes.post(
  ...
  async (req, res) => {
    ...
    const alreadyLinked = store.alunos.find(
      (item) => matchesCpf(item, cpf),
    );

    const student = store.alunos.find(
      (item) =>
        item.id === req.body.alunoId &&
        item.ativo !== false,
    );
    ...
    if (alreadyLinked)
      return res.status(409).json({
        error:
          "Este CPF já está vinculado a outro aluno.",
      });

    if (!student || student.cpfHash)
      return res.status(409).json({
        error: "Aluno indisponível para vínculo.",
      });
    ...
    return res.json({
      state: "linked",
      student: publicKioskStudent(student),
      sessions: openSessions(store).filter(
        (session) =>
          (student.turmaIds || []).includes(
            session.turmaId,
          ),
      ),
    });
...
routes.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(409).json({
        error: "Este CPF já possui vínculo.",
      });
...
async function recordAttendance(
  ...
  const classItem = store.turmas.find(
    (item) => item.id === session.turmaId,
  );
...
routes.post(
  ...
  async (req, res) => {
    ...
    const storeClassIds = new Set(
      store.turmas.map((item) => item.id),
    );

    const turmaIds = Array.isArray(
      req.body.turmaIds,
    )
      ? req.body.turmaIds.filter(
          (id) =>
            typeof id === "string" &&
            storeClassIds.has(id),
        )
      : [];
    ...
    if (
      ...
      return res.status(409).json({
        error:
          "Já existe um aluno ativo com esse nome.",
      });
...
routes.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(409).json({
        error:
          "Já existe uma turma com esse código.",
      });
...
routes.put(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(409).json({
        error:
          "Já existe um professor ativo com esse nome.",
      });
    ...
    if (cpf) {
      ...
      if (
        ...
        return res.status(409).json({
          error:
            "Este CPF já está vinculado a outro professor.",
        });
    ...
    store.turmas = store.turmas.map(
      (item) =>
        item.professorId === teacher.id
          ? {
              ...item,
              professor: name,
            }
...
routes.delete(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(409).json({
        error:
          "Não é possível apagar um professor ligado a uma turma ativa.",
      });
...
routes.put(
  ...
  async (req, res) => {
    ...
    const horarios = validatedSchedules(
      req.body,
    );
    ...
    if (
      ...
      return res.status(422).json({
        error:
          "Preencha curso, professor, dia e horários corretamente.",
      });
  ...
  async (req, res) => {
    ...
    if (
      store.alunos.some(
        (item) =>
          item.ativo !== false &&
          (item.turmaIds || []).includes(
            classItem.id,
          ),
      ) ||
      store.checkins.some(
        (item) =>
          item.turmaId === classItem.id,
      )
    )
      return res.status(409).json({
        error:
    ...
    store.turmas = store.turmas.filter(
    ...
    store.config = {
      ...
      tolDepois: Math.min(
        ...
        Math.max(
          ...
          Number(
            req.body.tolDepois ??
              store.config.tolDepois,
          ),
        ),
      ),

      inicioControle: String(
        req.body.inicioControle ||
          store.config.inicioControle,
      ),
    };
 ...
 *
 * Desvincular CPF:
 * DELETE /api/admin/students/:id/cpf
 *
 * Desativar aluno:
 * DELETE /api/admin/students/:id
 *
 * O aluno não é apagado fisicamente.
 * O histórico de presença permanece.
 */
...
routes.put(
  ...
  async (req, res) => {
    ...
    const turmaIds =
      Array.isArray(req.body.turmaIds)
        ? req.body.turmaIds.filter(
            (id) =>
              typeof id === "string" &&
              storeClassIds.has(id),
    ...
    /*
     * O CPF no frontend vem vazio quando
     * o administrador não deseja alterar o CPF.
     *
     * Portanto:
     * - CPF vazio = mantém o CPF atual
     * - CPF preenchido = valida e substitui
     */
    if (cpf) {
      if (!validCpf(cpf))
        return res.status(422).json({
    ...
    student.nome = name;
    student.tel = phone;
    student.turmaIds = turmaIds;

    student.desde = String(
      req.body.desde ||
        student.desde ||
        dateKey(now()),
    );
 ...
 * - presença
 *
 * O CPF puro nunca é armazenado.
 * Apenas o cpfHash é removido.
 */
routes.delete(
  "/api/admin/students/:id/cpf",
  adminLimiter,
  requireAdmin,
  async (req, res) => {
    const store = await readStore();
 ...
 * faz exclusão lógica.
 *
 * O cadastro permanece no banco para
...
routes.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(409).json({
        error:
          "A importação inicial exige uma base vazia.",
      });
    ...
    res.status(201).json({
      ok: true,
      alunos:
        imported.alunos.length,
      turmas:
        imported.turmas.length,
    });
  },
);
